...
export type KMSProvider = typeof KMS_PROVIDERS[keyof typeof KMS_PROVIDERS];
...
export interface CryptoKMSOptions {
  kmsProvider?: KMSProvider | string;
  kmsConfig?: {
    masterKey?: Buffer;
 ...
 */
interface KMSClient {
  encryptDataKey(plaintextDek: Buffer, keyId: string): Promise<Buffer>;
  decryptDataKey(encryptedDek: Buffer, keyId: string): Promise<Buffer>;
...
class LocalKMSClient implements KMSClient {
  ...
  private peppers: Record<string, string>;

  constructor(config: { masterKey?: Buffer; peppers?: Record<string, string> }) {
    this.masterKey = config.masterKey || crypto.randomBytes(32);
    this.peppers = config.peppers || {};
  }

  async encryptDataKey(plaintextDek: Buffer, keyId: string): Promise<Buffer> {
    // Simple local encryption using master key
    const cipher = crypto.createCipheriv('aes-256-gcm', this.masterKey, crypto.randomBytes(12));
    const encrypted = Buffer.concat([cipher.update(plaintextDek), cipher.final()]);
    const tag = cipher.getAuthTag();
    return Buffer.concat([encrypted, tag]);
  }

  async decryptDataKey(encryptedDek: Buffer, keyId: string): Promise<Buffer> {
    // Simple local decryption
    const tag = encryptedDek.slice(-16);
    const encrypted = encryptedDek.slice(0, -16);
    const decipher = crypto.createDecipheriv('aes-256-gcm', this.masterKey, crypto.randomBytes(12));
 ...
 * CryptoKMS Class
 */
export class CryptoKMS {
  static KMS_PROVIDERS = KMS_PROVIDERS;

  private kmsClient: KMSClient;
  private pepperCache: string | null = null;
  private pepperSecretId: string | null;
  private cmkKeyId: string | null;
  private argon2Params: any;
   ...
   */
  async hashPassword(plainPassword: string): Promise<{ hash: string }> {
  ...
  async encryptField(
    ...
    options?: {
      ...
      existingDekKmsKeyId?: string;
      existingDekVersion?: number;
    }
  ...
  async rotateMasterKey(
    oldKeyId: string,
    newKeyId: string,
    oldEncryptedDek: string | Buffer
  ): Promise<{ encryptedDek: string; dekMeta: EncryptionResult['dekMeta'] }> {
