...
export default function AdminMagicLinkPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setMessage('If an admin account exists, a magic link has been sent to your email.');
    } catch (err) {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gray-50 p-6">
      <div className="w-full max-w-md rounded-2xl bg-white p-6 shadow ring-1 ring-gray-200">
        <h1 className="text-2xl font-semibold text-gray-900 mb-2">Admin magic link</h1>
        <p className="text-sm text-gray-500 mb-6">Enter the admin email and we'll send a one-time sign-in link.</p>
